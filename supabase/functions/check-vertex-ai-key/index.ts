...
serve(async (req) => {
  ...
  try {
    ...
    const model = "gemini-1.5-pro"; // Using stable model ID for connection test
    ...
    if (!vertexData.candidates || vertexData.candidates.length === 0) {
      throw new Error("Vertex AI returned a valid response but with no content. Check model and permissions.");
    }
    ...
    console.error('Error in check-vertex-ai-key function:', error.message);
