...
interface AiLogDetailDialogProps {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  log: AiLog | null;
}

export const AiLogDetailDialog = ({ isOpen, onOpenChange, log }: AiLogDetailDialogProps) => {
  if (!log) return null;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>Chi tiết Nhật ký AI</DialogTitle>
          <DialogDescription>
            Xem lại nhật ký và prompt system đã được sử dụng để tạo ra câu trả lời này.
          </DialogDescription>
        </DialogHeader>
