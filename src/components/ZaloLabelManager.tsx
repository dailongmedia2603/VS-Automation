...
export const ZaloLabelManager = () => {
  const [labels, setLabels] = useState<ZaloLabel[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [currentLabel, setCurrentLabel] = useState<Partial<ZaloLabel> | null>(null);
  const [labelToDelete, setLabelToDelete] = useState<ZaloLabel | null>(null);
  ...
  const handleEdit = (label: ZaloLabel) => {
  ...
  const handleDelete = () => {
    ...
    setIsSaving(true);
    supabase.from('zalo_labels').delete().eq('id', labelToDelete.id).then(({ error }) => {
      if (error) {
        showError("Xóa thẻ thất bại: " + error.message);
      } else {
  ...
  const handleSave = async () => {
    if (!currentLabel || !currentLabel.name) {
      showError("Tên thẻ không được để trống.");
      return;
    }
    setIsSaving(true);
    const { error } = await supabase.from('zalo_labels').upsert({
      id: currentLabel.id,
    ...
    if (error) {
      showError("Lưu thẻ thất bại: " + error.message);
    } else {
      showSuccess("Đã lưu thẻ thành công!");
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Thẻ Zalo</CardTitle>
        <CardDescription>Sử dụng thẻ Zalo giúp phân biệt các cuộc trò chuyện hoặc khách hàng.</CardDescription>
      </CardHeader>
      <CardContent>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Tùy chỉnh tên và màu sắc cho thẻ Zalo của bạn.
            </DialogDescription>
          </DialogHeader>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể được hoàn tác. Thẻ "{labelToDelete?.name}" sẽ bị xóa vĩnh viễn.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
