    ...
    <>
      ...
      <Dialog open={isProjectDialogOpen} onOpenChange={setIsProjectDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">{editingProject ? 'Sửa kế hoạch' : 'Tạo kế hoạch mới'}</DialogTitle>
            <DialogDescription>
              {editingProject ? 'Thay đổi tên cho kế hoạch của bạn.' : 'Nhập tên cho kế hoạch mới.'}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-2">
            <Label htmlFor="project-name">Tên kế hoạch</Label>
            <Input
              id="project-name"
              value={projectName}
              onChange={(e) => setProjectName(e.target.value)}
              placeholder="VD: Kế hoạch ra mắt sản phẩm X"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể hoàn tác. Kế hoạch "{projectToDelete?.name}" sẽ bị xóa vĩnh viễn.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
