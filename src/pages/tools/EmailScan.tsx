...
import { useEmailScanProjects, useCreateEmailScanProject, useUpdateEmailScanProject, useDeleteEmailScanProject } from '@/hooks/useTools';
import { EmailScanProject } from '@/api/tools';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { Folder, PlusCircle, Search, List, LayoutGrid, ChevronDown, Loader2, Mail } from 'lucide-react';
...
const EmailScan = () => {
  ...
  const { data: projects = [], isLoading } = useEmailScanProjects();
  const createProject = useCreateEmailScanProject();
  const updateProject = useUpdateEmailScanProject();
  const deleteProject = useDeleteEmailScanProject();

  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [isProjectDialogOpen, setIsProjectDialogOpen] = useState(false);
  const [editingProject, setEditingProject] = useState<EmailScanProject | null>(null);
  const [projectName, setProjectName] = useState('');
  const [projectToDelete, setProjectToDelete] = useState<EmailScanProject | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');

  const filteredProjects = useMemo(() => {
    return projects.filter((p: EmailScanProject) =>
      p.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [projects, searchTerm]);

  const displayProjects: DisplayProject[] = useMemo(() => {
    return filteredProjects.map((p: EmailScanProject) => ({
      id: p.id,
      name: p.name,
  ...
  const stats = useMemo(() => {
    return [
      ...
      { title: 'Tổng số email', value: String(projects.reduce((acc: number, p: EmailScanProject) => acc + (p.results_count || 0), 0)), icon: Mail, color: 'bg-red-500' },
    ];
  }, [projects]);

  const handleOpenDialog = (project: EmailScanProject | null = null) => {
  ...
  const handleOpenDeleteDialog = (project: EmailScanProject) => {
  ...
  const renderProjectView = () => {
    ...
    const projectActions = (project: EmailScanProject) => ({
    ...
    <>
      <main className="flex-1 space-y-8 p-6 sm:p-8 bg-slate-50">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Quét Email</h1>
            <p className="text-muted-foreground mt-2 max-w-2xl">
              Quản lý các dự án quét email từ bình luận bài viết Facebook.
            </p>
          </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4 space-y-2">
            ...
            <Input
              ...
              placeholder="VD: Dự án quét email ABC"
