...
const EmailScan = () => {
  ...
  const fetchProjects = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase.rpc('get_email_scan_projects_with_creator');
  ...
  const stats = useMemo(() => {
    return [
      ...
      { title: 'Tổng số email', value: String(projects.reduce((acc, p) => acc + p.results_count, 0)), icon: Mail, color: 'bg-red-500' },
    ];
  }, [projects]);
  ...
  const handleDeleteProject = async () => {
    ...
    const { error } = await supabase.from('email_scan_projects').delete().eq('id', projectToDelete.id);
    if (error) {
